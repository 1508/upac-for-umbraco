...
<xsl:stylesheet
  ...
  exclude-result-prefixes="msxml umbraco.library Exslt.ExsltCommon Exslt.ExsltDatesAndTimes Exslt.ExsltMath Exslt.ExsltRegularExpressions Exslt.ExsltStrings Exslt.ExsltSets upac ">
	...
	<xsl:template match="/">
		<xsl:if test="$currentPage/showPublishedDate = '1'">
			<div class="pageinfo-top">
				<div class="pageinfo-published">
					<xsl:value-of select="umbraco.library:GetDictionaryItem('Global-Pageinfo-Published')"/>
					<xsl:value-of select="upac:FormatIsoDateShort($currentPage/date)" />
				</div>
			</div>
		</xsl:if>
	</xsl:template>
