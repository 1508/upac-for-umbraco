...
<xsl:template match="/">
	<xsl:variable name="value" select="$currentPage/ancestor-or-self::*[@level=1]/ConfigurationContainer/ConfigurationGoogle/webPropertyID" />
	<xsl:if test="string($value)">
		<script type="text/javascript">
			var _gaq = _gaq || [];
			_gaq.push(['_setAccount', '<xsl:value-of select="$value" />']);
