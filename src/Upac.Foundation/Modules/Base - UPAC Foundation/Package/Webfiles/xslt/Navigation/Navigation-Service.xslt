...
<xsl:param name="currentPage"/>
    ...
    <xsl:variable name="includeFrontpage" select="string(/macro/includeFrontpage) = '1'"/>
...
<xsl:template match="/">
    ...
    <div class="servicenav">
        ...
        <p class="accessibility">
          <strong>
            <xsl:value-of select="umbraco.library:GetDictionaryItem('Accessibility-ServiceNavigation')"/>
          </strong>
        </p>
      <ul>
        <xsl:if test="$includeFrontpage">
          <li>
            <a href="/">
              <xsl:value-of select="umbraco.library:GetDictionaryItem('Global-Frontpage')"/>
            </a>
          </li>
        </xsl:if>
        <xsl:variable name="nodeContainer" select="$currentPage/ancestor-or-self::*[@level=1]/FolderNavigationService" />
