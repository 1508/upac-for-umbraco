namespace Upac.NantTasks
{
    ...
    /// </summary>
    [TaskName("mergeumbracopackagedefinitionfiles")]
    public class MergeUmbracoPackageXmlFile : Task
    {
        #region Properties
        ...
        {
            Project.Log(Level.Info, "Starting to merge umbraco definition files");
            ...
            // files
            Merge(sourceDoc, targetDoc, "/umbPackage/files/file", "/umbPackage/files");

            // Documents
            //TODO

            // DocumentTypes
            Merge(sourceDoc, targetDoc, "/umbPackage/DocumentTypes/DocumentType", "/umbPackage/DocumentTypes");

            // Templates
            Merge(sourceDoc, targetDoc, "/umbPackage/Templates/Template", "/umbPackage/Templates");

            // Stylesheets
            //TODO

            // Macros
            Merge(sourceDoc, targetDoc, "/umbPackage/Macros/macro", "/umbPackage/Macros");

            // DictionaryItems
            Merge(sourceDoc, targetDoc, "/umbPackage/DictionaryItems/DictionaryItem", "/umbPackage/DictionaryItems");

            // Languages
            //TODO

            // Actions
            Merge(sourceDoc, targetDoc, "/umbPackage/Actions/Action", "/umbPackage/Actions");

            // DataTypes
            Merge(sourceDoc, targetDoc, "/umbPackage/DataTypes/DataType", "/umbPackage/DataTypes");
        ...
        private void Merge(XmlDocument sourceDoc, XmlDocument targetDoc, string sourceXPath, string targetXPath)
        {
            XmlNodeList sourceNodes = sourceDoc.SelectNodes(sourceXPath);
            Project.Log(Level.Info, string.Format("Found {0} nodes(s) in source package file", sourceNodes.Count));
            ...
            if (sourceNodes.Count >= 1)
            {
                ...
                else
                {
                    ...
                    Project.Log(Level.Info, "Target node found!");
